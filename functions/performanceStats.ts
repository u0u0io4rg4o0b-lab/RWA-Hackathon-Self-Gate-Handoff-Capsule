...
const perfStats: Array<{ timestamp: string; endpoint: string; time_ms: number; mode?: string }> = [];

export const performanceStats = onRequest({ region: REGION, timeoutSeconds: TIMEOUT_SECONDS }, async (req, res) => {
  ...
  perfStats.forEach(stat => {
    if (!byEndpoint[stat.endpoint]) byEndpoint[stat.endpoint] = [];
  ...
  perfStats.forEach(stat => {
    if (stat.mode) {
      if (!byMode[stat.mode]) byMode[stat.mode] = [];
...
// 匯出函數以記錄性能數據（供其他端點使用）
export function recordPerformance(endpoint: string, timeMs: number, mode?: string) {
  perfStats.push({
    timestamp: new Date().toISOString(),
    endpoint,
    time_ms: timeMs,
