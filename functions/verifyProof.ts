...
function byteLengthOfJson(x: unknown): number {
  try { return Buffer.byteLength(JSON.stringify(x ?? {}), 'utf8'); }
  catch { return Infinity; } // 無法序列化就當作超限
}
...
function evaluateRules(proofData: any, requirements: string[]): RuleResult[] {
  ...
  for (const rule of requirements) {
    ...
    if (rule === 'age>=18') {
      const age = Number((proofData && (proofData as any).age) ?? NaN);
...
// ======================== 主處理 ===========================
export const verifyProof = onRequest(
